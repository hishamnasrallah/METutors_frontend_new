...
export class CometChatIncomingDirectCallComponent
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      this.incomingDirectCall &&
      this.incomingDirectCall.type == enums.CALL_TYPE_DIRECT &&
      localStorage.getItem('isIncomingCall') != 'accepted'
    ) {
      ...
      CometChat.addMessageListener(
        ...
        new CometChat.MessageListener({
          onCustomMessageReceived: (textMessage: any) => {
            if (
              textMessage &&
              textMessage.type == enums.CALL_TYPE_DIRECT &&
              localStorage.getItem('isIncomingCall') != 'accepted'
            ) {
  ...
  joinDirectCall() {
    ...
    let sessionID =
      this.incomingDirectCall.data.customData.sessionId ||
      this.incomingDirectCall.data.customData.sessionID;
    let audioOnly = false;
  ...
  ngOnInit() {
    ...
    window.addEventListener('storage', () => {
      if (
        (localStorage.getItem('isIncomingCall') == 'ignored' ||
          localStorage.getItem('isIncomingCall') == 'accepted') &&
        this.incomingCall
      ) {
        this.incomingCall = null;
